...
class TestAnimationPresetTools:
    ...
    async def test_preset_pulse_forwards_property_values(self, mcp_stack):
        client, plugin = mcp_stack

        async def respond():
            cmd = await plugin.recv_command()
            assert cmd["command"] == "animation_preset_pulse"
            assert cmd["params"]["property"] == "modulate:a"
            assert cmd["params"]["from_value"] == 0.2
            assert cmd["params"]["to_value"] == 1.0
            assert cmd["params"]["loop_mode"] == "linear"
            await plugin.send_response(
                cmd["request_id"],
                {
        ...
        await task
        assert result.data["property"] == "modulate:a"
        assert result.data["loop_mode"] == "linear"

    async def test_preset_orbit_dispatches(self, mcp_stack):
        client, plugin = mcp_stack

        async def respond():
            cmd = await plugin.recv_command()
            assert cmd["command"] == "animation_preset_orbit"
            assert cmd["params"]["radius"] == 25.0
            assert cmd["params"]["clockwise"] is True
            await plugin.send_response(
                cmd["request_id"],
                {
                    "player_path": "/Main/AP",
                    "animation_name": "orbit",
                    "radius": 25.0,
                    "clockwise": True,
                    "length": 2.0,
                    "loop_mode": "none",
        ...
        await task
        assert result.data["animation_name"] == "orbit"
