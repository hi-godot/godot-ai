...
def test_main_widens_http_bind_only_when_allow_host_set(monkeypatch):
    ...
    ## Non-default --ws-port: the preflight really binds the WS port, and
    ## the default 9500 is held by any live dev-server/editor on this
    ## machine — without the override the test fails on environment, not
    ## code.
    godot_ai.main(
        [
            "--transport",
...
def test_main_does_not_widen_bind_without_allow_host(monkeypatch):
    ...
    ## Non-default --ws-port for the same live-dev-server reason as above.
    godot_ai.main(["--transport", "streamable-http", "--port", "8123", "--ws-port", "9557"])
