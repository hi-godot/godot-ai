...
def test_parse_rejects_core_only_batch_domain():
    ## `batch` became all-core in #194 — same hard-fail rule as `session`.
...
def test_excludable_domains_drops_all_core_domains():
...
def test_create_server_preserves_core_when_core_bearing_domain_excluded():
    ## Excluding `node` drops node_find, node_duplicate, … but keeps
    ## the core node CRUD set (create/delete/set_property/reparent/rename
    ## plus get_properties). Same idea for editor/scene.
