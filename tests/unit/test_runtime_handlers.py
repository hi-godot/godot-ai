...
async def test_animation_preset_pulse_forwards_property_and_values():
    """The property/from_value/to_value/loop_mode extension reaches the plugin."""
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
    await animation_handlers.animation_preset_pulse(
        runtime,
        player_path="/Main/AP",
        target_path="Panel",
        property="modulate:a",
    ...
    assert client.calls[-1]["command"] == "animation_preset_pulse"
    params = client.calls[-1]["params"]
    assert params["property"] == "modulate:a"
    assert params["from_value"] == 0.2
    assert params["to_value"] == 1.0
    assert params["loop_mode"] == "linear"


async def test_animation_preset_pulse_omits_unset_values():
    """The default payload keeps the scale shortcut and no value keys."""
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
    await animation_handlers.animation_preset_pulse(
        runtime, player_path="/Main/AP", target_path="Panel"
    )
    params = client.calls[-1]["params"]
    assert "from_value" not in params
    assert "to_value" not in params
    assert params["property"] == "scale"
    assert params["loop_mode"] == "none"


async def test_animation_preset_continuous_handlers_forward_params():
    """bounce/orbit/sweep/drift reach the plugin with their own params."""
    client = StubClient()
    runtime = DirectRuntime(registry=SessionRegistry(), client=client)
    await animation_handlers.animation_preset_bounce(
        runtime, player_path="/Main/AP", target_path="Panel", intensity=0.2
    )
    assert client.calls[-1]["command"] == "animation_preset_bounce"
    assert client.calls[-1]["params"]["intensity"] == 0.2
    await animation_handlers.animation_preset_orbit(
        runtime, player_path="/Main/AP", target_path="Panel", radius=25.0, loop_mode="linear"
    )
    assert client.calls[-1]["command"] == "animation_preset_orbit"
    assert client.calls[-1]["params"]["radius"] == 25.0
    assert client.calls[-1]["params"]["loop_mode"] == "linear"
    await animation_handlers.animation_preset_sweep(
        runtime, player_path="/Main/AP", target_path="Panel", turns=2.0, clockwise=False
    )
    assert client.calls[-1]["command"] == "animation_preset_sweep"
    assert client.calls[-1]["params"]["turns"] == 2.0
    assert client.calls[-1]["params"]["clockwise"] is False
    await animation_handlers.animation_preset_drift(
        runtime, player_path="/Main/AP", target_path="Panel", axis="y", distance=12.0
    )
    assert client.calls[-1]["command"] == "animation_preset_drift"
    assert client.calls[-1]["params"]["axis"] == "y"
    assert client.calls[-1]["params"]["distance"] == 12.0
    ## Omitted optional floats stay out of the payload.
    await animation_handlers.animation_preset_orbit(
        runtime, player_path="/Main/AP", target_path="Panel"
    )
    assert "radius" not in client.calls[-1]["params"]
    await animation_handlers.animation_preset_drift(
        runtime, player_path="/Main/AP", target_path="Panel"
    )
    assert "distance" not in client.calls[-1]["params"]
