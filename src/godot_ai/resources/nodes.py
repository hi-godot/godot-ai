...
def register_node_resources(mcp: FastMCP) -> None:
    ...
    async def get_node_properties(ctx: Context, path: str) -> str:
        ...
        return await safe_json(node_handlers.node_get_properties(runtime, path=_normalize(path)))
    ...
    async def get_node_children(ctx: Context, path: str) -> str:
        ...
        runtime = DirectRuntime.from_context(ctx)
        return await safe_json(node_handlers.node_get_children(runtime, path=_normalize(path)))
    ...
    async def get_node_groups(ctx: Context, path: str) -> str:
        ...
        runtime = DirectRuntime.from_context(ctx)
        return await safe_json(node_handlers.node_get_groups(runtime, path=_normalize(path)))
