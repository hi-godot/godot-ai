...
CORE_BEARING_DOMAINS: frozenset[str] = frozenset(
    {"session", "editor", "scene", "node", "batch"}
)

## The core tools that survive any exclusion. Displayed as a disabled
## "Core" row in the plugin UI. Core = observe + the CRUD primitives every
## write tool composes onto, so the always-on tier is a viable minimum
## agent surface, not just the minimum readable one (#194).
...
## Domains the user can toggle off. `session` and `batch` have no non-core
## tools, so excluding either would be a no-op; we reject it up front so
## an accidental `--exclude-domains session` doesn't silently do nothing.
