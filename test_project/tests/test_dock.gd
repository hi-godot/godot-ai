...
func test_unsupported_remote_access_heading_does_not_claim_server_exited() -> void:
	...
	assert_true(dock._server_blocks_client_health())
	assert_true(dock._crash_panel.visible)
	assert_eq(dock._crash_output.get_parsed_text(), "Use an IPv4 allowlist or disable remote access.")
	assert_false(dock._port_picker_panel.visible)
	assert_false(dock._crash_restart_btn.visible)
