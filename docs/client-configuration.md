...
from a fallback path. If no wildcard package matches and no file exists, the
first non-wildcard candidate is the deterministic create target. Ambiguous
wildcard groups return a structured resolution error that status, Configure,
Remove, and manual instructions surface to the user. Claude Desktop uses this
on Windows to select its MSIX `LocalCache/Roaming` config without hardcoding
the Store publisher hash. With no Store package it uses normal roaming; the
dock never writes divergent copies.
