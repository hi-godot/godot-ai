...
- **WS trust boundary**: the editor↔server WebSocket (port 9500) has **no authentication** — the handshake carries only session_id/version/path, all chooseable by the connecting process, and the first registered session becomes the active session for un-pinned tool calls. The only control is loopback-only binding: the WS port always binds `127.0.0.1` and `--allow-host` deliberately does NOT widen it (see `transport/websocket.py::start`). Any process that can connect to localhost is trusted; defense in depth on top of that is the unguessable `uuid4` request ids plus session-scoped response correlation (a reply is only accepted from the connection its command was sent to — #690). A per-launch handshake token is a possible future hardening (compat-gated protocol change, maintainer decision — #690 finding 4). Never bind the WS port beyond loopback.
